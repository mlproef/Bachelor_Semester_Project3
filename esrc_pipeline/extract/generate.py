"""Single LLM generate() wrapper for Ollama (OpenAI-compatible /v1 API)."""
...
def get_client(
    *,
    ...
    """Build an OpenAI client pointed at Ollama (OpenAI-compatible /v1 API).

    max_retries defaults to 0 so 400/context-exceeded (and litellm 500 wraps)
    are never blindly resent in a loop by the SDK.
    """
    resolved_base = (base_url or os.getenv("OLLAMA_URL") or "").rstrip("/")
    if not resolved_base:
        raise RuntimeError(
            "OLLAMA_URL is not set. Example: export OLLAMA_URL=http://127.0.0.1:8000/v1"
        )
    resolved_key = api_key or os.getenv("OLLAMA_API_KEY") or "EMPTY"
    return OpenAI(
...
def generate(
    ...
    *,
    ...
    thinking templates. Pass None to leave server defaults unchanged.
