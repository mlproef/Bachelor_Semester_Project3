#!/usr/bin/env python3
"""Run ESRC Reason for Condition C.

Needs::
  results/c/extract/query/*.txt
  results/c/extract/candidates/*.txt
  results/c/search/search_top15.csv

Writes::
  results/c/reason/reason_predictions.jsonl
  results/c/reason/reason_metrics.json

Usage:
  python esrc_pipeline/scripts/run_reason_c.py --dry-run --limit-queries 2
  python esrc_pipeline/scripts/run_reason_c.py
"""
...
def main() -> None:
    ...
    p.add_argument(
        ...
        default=None,
        help="Whose candidate extracts to pick from (default: C)",
    )
    args = p.parse_args()
