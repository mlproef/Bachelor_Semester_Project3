#!/usr/bin/env python3
"""Run ESRC Search for Condition B.

Needs Extract query summaries::
  esrc_pipeline/results/b/extract/query/*.txt

Gallery is Extract candidate summaries (same condition by default)::
  esrc_pipeline/results/b/extract/candidates/*.txt

One-sided (B queries vs raw / baseline candidates)::
  python esrc_pipeline/scripts/run_search_b.py --gallery-condition baseline

Writes::
  esrc_pipeline/results/b/search/search_top15.csv
  or, if gallery differs:
  esrc_pipeline/results/b/search_vs_baseline/search_top15.csv

Usage:
  python esrc_pipeline/scripts/run_search_b.py --dry-run
  python esrc_pipeline/scripts/run_search_b.py --limit-queries 5
"""
...
def main() -> None:
    ...
    p.add_argument(
        ...
        default=None,
        help="Whose candidate extracts to search against (default: B)",
    )
    args = p.parse_args()
