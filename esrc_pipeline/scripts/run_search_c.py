#!/usr/bin/env python3
"""Run ESRC Search for Condition C.

Needs Extract query summaries::
  esrc_pipeline/results/c/extract/query/*.txt

Gallery is Extract candidate summaries (same condition by default)::
  esrc_pipeline/results/c/extract/candidates/*.txt

One-sided (C queries vs raw / baseline candidates)::
  python esrc_pipeline/scripts/run_search_c.py --gallery-condition baseline

Writes::
  esrc_pipeline/results/c/search/search_top15.csv
  or, if gallery differs:
  esrc_pipeline/results/c/search_vs_baseline/search_top15.csv

Usage:
  python esrc_pipeline/scripts/run_search_c.py --dry-run
  python esrc_pipeline/scripts/run_search_c.py --limit-queries 5
"""
...
def main() -> None:
    ...
    p.add_argument(
        ...
        default=None,
        help="Whose candidate extracts to search against (default: C)",
    )
    args = p.parse_args()
