#!/usr/bin/env python3
"""Run ESRC Reason for So2 control C vs a candidate gallery.

Needs::
  results/so2_c/extract/query/*.txt
  results/baseline/extract/candidates/*.txt
  results/so2_c/search_vs_baseline/search_top15.csv

Writes::
  results/so2_c/reason_vs_baseline/reason_predictions.jsonl

Usage:
  python esrc_pipeline/scripts_so2/run_reason_c.py --dry-run --limit-queries 2
  python esrc_pipeline/scripts_so2/run_reason_c.py --gallery-condition baseline
"""
...
def main() -> None:
    ...
    p.add_argument(
        ...
        default="baseline",
        help="Whose candidate extracts to pick from (default: baseline)",
    )
    args = p.parse_args()
