#!/usr/bin/env python3
"""
So1 BGE — run Condition A only (all query users by default).

Usage (from repo root):
  python so1_bge/scripts/run_A_bge.py
  python so1_bge/scripts/run_A_bge.py --limit-users 5
  python so1_bge/scripts/run_A_bge.py --no-resume
"""
...
def run_condition_A(
    *,
    ...
    """
    Score Condition A: original ↔ spaCy NER anonymized queries.

    Appends one JSON line per user to bge_condition_A_per_user.jsonl,
    then writes aggregate summary to bge_condition_A.json.
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    per_user_path = out_dir / "bge_condition_A_per_user.jsonl"
    summary_path = out_dir / "bge_condition_A.json"

    pairs = load_condition_pairs("A")
    ...
    print(
        f"[A] model={model_name} device={device} "
    ...
    with per_user_path.open("a", encoding="utf-8") as out_f:
        for pair in tqdm(pending, desc="condition A", unit="user"):
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="So1 BGE Condition A only")
