#!/usr/bin/env python3
"""
So1 BGE — run Condition C only (all query users by default).

Needs condition_c/anonymized_c/query/ with user_*_query.jsonl (same layout as A/B).

Usage (from repo root):
  python so1_bge/scripts/run_C_bge.py
  python so1_bge/scripts/run_C_bge.py --limit-users 5
  python so1_bge/scripts/run_C_bge.py --no-resume
"""
...
def run_condition_C(
    *,
    ...
    """
    Score Condition C: original ↔ Staab-style anonymized queries.

    Appends one JSON line per user to bge_condition_C_per_user.jsonl,
    then writes aggregate summary to bge_condition_C.json.

    By default skips profiles identical to the original (placeholders).
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    per_user_path = out_dir / "bge_condition_C_per_user.jsonl"
    summary_path = out_dir / "bge_condition_C.json"

    pairs = load_condition_pairs("C")
    skipped_identical = 0
    if require_changed:
        before = len(pairs)
        pairs = filter_real_anonymizations(pairs)
        skipped_identical = before - len(pairs)
    ...
    print(
        f"[C] model={model_name} device={device} "
    ...
    with per_user_path.open("a", encoding="utf-8") as out_f:
        for pair in tqdm(pending, desc="condition C", unit="user"):
    ...
    summary = {
        ...
        "device": device,
        "n_users": len(ordered),
        "skipped_identical_profiles": skipped_identical,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="So1 BGE Condition C only")
    ...
    p.add_argument(
        "--keep-identical",
        action="store_true",
        help="Do not skip profiles identical to original",
    )
...
def main() -> None:
    ...
    run_condition_C(
        ...
        require_changed=not args.keep_identical,
