#!/usr/bin/env python3
"""
So2 matched — Condition C rate only.

Usage (from repo root):
  python so2_matched/scripts/run_C.py
  python so2_matched/scripts/run_C.py --limit-users 5
  python so2_matched/scripts/run_C.py --force
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="So2 matched POS degradation — match C")
    p.add_argument("--lexicon", type=Path, default=DEFAULT_LEXICON)
    p.add_argument("--limit-users", type=int, default=None)
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--force", action="store_true")
    p.add_argument("--out-dir", type=Path, default=out_dir("C"))
    return p.parse_args()
